<template>
    ...
    <el-dialog draggable v-model="dialogVisible7" title="1号U型辊架状态" width="50%">
        ...
        <div class="bottombox">
            <div v-if="istatus == 3" class="btfont">请确保C02 C03工位存在空辊框 可以通过AGV半自动任务 搬运空辊框到C02 或者C03工位</div>
            <div v-if="istatus == 1" class="my-2 flex items-center text-sm">
                <el-radio-group v-model="radio" class="ml-4">
                    <el-radio style="color: black" label="13">下发至13号磨床</el-radio>
...
const slot = (row: any, i: any) => {
    switch (i) {
        ...
        case 2:
            ...
            // axios.get(`http://192.2.25.61/api/web/UpdateRollInfo?RollNo=${row.rollId}&Diameter=${row.diameter}&Roll_Seed=${row.rollType}&Grinder_No=${row.grindNum}&SurfaceState=${row.tableFace}&Reason=${row.dremarks}&Result=${row.testResult}&Taper=${}&Plant_Code=${}`).then((res: any) => {
...
onMounted(() => {
    ...
    axios.get('http://192.2.25.61/api/web/GetProgramNoByRollType?GrindNo=13,14').then((res: any) => {
